...
with open('model/CustomStackedModel.pkl', 'rb') as f:
...
def predicted_value(patient_symptoms):
    ...
    # Apply preprocessing
    i_vector = scaler.transform([i_vector])
    i_vector = selector.transform(i_vector)

    pred = model.predict(i_vector)[0]
...
def home():
    if request.method == 'POST':
        ...
        if not symptoms or symptoms.lower() == "symptoms":
            message = "Please enter your symptoms (comma-separated)."
            return render_template('index.html', message=message)

        # split & clean raw input
        patient_symptoms = [s.strip() for s in symptoms.split(',') if s.strip()]
        if not patient_symptoms:
            message = "No symptoms detected in your input. Please type symptoms separated by commas."
            return render_template('index.html', message=message)

        corrected_symptoms = []
        unmatched = []         # symptoms with no confident match
        suggestions = {}       # store suggestions for unmatched symptoms

        for symptom in patient_symptoms:
            # first try exact normalized match (to keep original behavior)
            s_norm = symptom.lower().replace(" ", "_")
            if s_norm in symptoms_list_processed:            # exact match by normalization
                corrected_symptoms.append(s_norm)
                continue

            # fuzzy match against known symptoms (use keys of symptoms_list_processed)
            best_match, score = process.extractOne(symptom, list(symptoms_list_processed.keys()))
            if score >= 80:
                corrected_symptoms.append(best_match)
            else:
                unmatched.append(symptom)
                # keep top 3 fuzzy suggestions to show to user
                top_suggestions = [t for t,_ in process.extract(symptom, list(symptoms_list_processed.keys()), limit=3)]
                suggestions[symptom] = top_suggestions

        # If no corrected symptoms found -> ask user to re-enter
        if len(corrected_symptoms) == 0:
            # Build friendly message listing unmatched + suggestions
            msg_lines = []
            msg_lines.append("We couldn't confidently match any of the symptoms you entered.")
            if unmatched:
                msg_lines.append("Unmatched symptom(s): " + ", ".join(unmatched))
                # show suggestions if available
                sug_lines = []
                for u in unmatched:
                    sgs = suggestions.get(u, [])
                    if sgs:
                        sug_lines.append(f"Suggestions for '{u}': {', '.join(sgs)}")
                if sug_lines:
                    msg_lines.append("You can try these suggestions: ")
                    msg_lines += sug_lines
            msg_lines.append("Please correct the spelling or try different symptom words and search again.")
            message = "<br>".join(msg_lines)
            return render_template('index.html', message=message, suggestions=suggestions)

        # If some symptoms matched but some unmatched -> show matched result and still warn/suggest
        if unmatched:
            warning_lines = []
            warning_lines.append("Some of your symptoms were recognized, others were not.")
            warning_lines.append("Unmatched: " + ", ".join(unmatched))
            # show top suggestion per unmatched
            for u in unmatched:
                sgs = suggestions.get(u, [])
                if sgs:
                    warning_lines.append(f"Did you mean: {u} → {', '.join(sgs)} ?")
            warning = "<br>".join(warning_lines)
        else:
            warning = None

        # Proceed to predict using corrected_symptoms (at least one)
        predicted_disease = predicted_value(corrected_symptoms)
        dis_des, precs, med_list, rec_diet, workout = information(predicted_disease)

        # Prepare precaution list and parse med/diet (same as before)
        my_precautions = []
        if isinstance(precs, (list, np.ndarray)) and len(precs) > 0:
            for i in precs[0]:
                my_precautions.append(i)

        # Convert medication/diet strings to python lists if necessary (they were stored as strings)
        try:
            medication_list = ast.literal_eval(med_list[0]) if isinstance(med_list, (list, np.ndarray)) and len(med_list)>0 else []
        except Exception:
            medication_list = med_list if isinstance(med_list, list) else []

        try:
            diet_list = ast.literal_eval(rec_diet[0]) if isinstance(rec_diet, (list, np.ndarray)) and len(rec_diet)>0 else []
        except Exception:
            diet_list = rec_diet if isinstance(rec_diet, list) else []
